...
enum WeatherKey2 {
  ...
  WEATHER_TEMPERATURE_KEY2 = 0x1,  // TUPLE_CSTRING
...
static void sync_tuple_changed_callback2(const uint32_t key, const Tuple* new_tuple, const Tuple* old_tuple, void* context) {
  char citytxt[]="Holte";
   char ttxt[]="Varmt";
  static int i = 42;
...
//   text_layer_set_text(s_temperature_layer, buf2);
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "Temp_layer: %s",  text_layer_get_text(s_city_layer));
  APP_LOG(APP_LOG_LEVEL_DEBUG, "Temp_layer: %s",  text_layer_get_text(s_windtxt_layer));
...
//      s_icon_bitmap = gbitmap_create_with_resource(WEATHER_ICONS[1]);
    ...
    APP_LOG(APP_LOG_LEVEL_DEBUG, "WTK %s",new_tuple->value->cstring);
